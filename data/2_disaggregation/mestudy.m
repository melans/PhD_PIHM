...
arg_list = argv ();
input_observed_daily=arg_list{1};
input_forecast_monthly=arg_list{2};
output_forecast_daily=arg_list{3};
...
%     for j = 1:length(longitude_series)
        ...
        % filename = strcat('data/','data_',num2str(lat_temp),'_',num2str(long_temp)); % on unix and windows,\ or / is different
        filename = strcat(input_observed_daily); % on unix and windows,\ or / is different
        ...
        if (judge == -1)
           % do nothing
            ...
            % filename = strcat('data/','monthly_',num2str(lat_temp),'_',num2str(long_temp)); % on unix and windows,\ or / is different
            filename = strcat(input_forecast_monthly); % on unix and windows,\ or / is different
            ...
            filename_out_prcp = strcat(output_forecast_daily);
