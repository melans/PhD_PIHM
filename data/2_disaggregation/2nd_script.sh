...
downscaling="../1_downscaling";
cptresults="$downscaling/tmp/cptresults";
...
case "$1" in
	"1")
		...
		MSG "collect coloumns 4,5,6,7 fom the covering grids of observed data (TINS_Grids)";
		# grids=`awk -F_ '{print "data_"$2"_"$3}' $TINs_Grids|sort|uniq|tr '\n' ' '`;
		grids=`awk -F_ '{print $2"_"$3}' $TINs_Grids|sort|uniq|tr '\n' ' '`;
		for g in ${grids[@]};do
			awk '{print $4,$5,$6,$7}' $downscaling/${observed_daily//$ME}/data_$g > $observed_daily/data_$g;
			MSGx "Generating grid ($g) for ($L) leadtime ==> ${forecast_monthly//$ME\/}/data_$g"_"$L";
			for L in {1..6};do
				yes $dummy|head -$((L-1))|xargs > tmp.tmp;
				pr -mts" " $cptresults/CV_$g"_"{01..12}_$L.txt|awk '$1*1>1960'|sed 's/\t/ /g'|cut -d" " -f$(seq -s, 2 2 24) >> tmp.tmp;
				tr '\n' ' ' <tmp.tmp|awk '{for(;++i<=NF;)printf $i (i%12?" ":"\n")}' > $forecast_monthly/data_$g"_"$L;
				# don't remove the last line for L=1
				if [[ $L -gt "1" ]];then
					sed -i '$d' $forecast_monthly/data_$g"_"$L;
				fi
			done
		done
		...
		# # octave medisagg.m data/input_observed_daily_30.9375_-84.9375 data/input_forecast_monthly_30.9375_-84.9375 data/output_forecast_daily_30.9375_-84.9375
		...
		# 		MSGx "Generating grid ($g) for ($L) leadtime ==> ${forecast_monthly//$ME\/}/data_$g"_"$L";
		# 		yes $dummy|head -$((L-1))|xargs > tmp.tmp;
		# 		pr -mts" " ../1_downscaling/tmp/cptresults/CV_$g*_$L.txt|awk '$1*1>1960'|sed 's/\t/ /g'|cut -d" " -f$(seq -s, 2 2 24) >> tmp.tmp;
		# 		tr '\n' ' ' <tmp.tmp|awk '{for(;++i<=NF;)printf $i (i%12?" ":"\n")}' > $forecast_monthly/data_$g"_"$L;
		# 		if [[ $L -gt "1" ]];then
		# 			sed -i '$d' $forecast_monthly/data_$g"_"$L;
		# 		fi
	...
	"2")
		...
		for monthly in $forecast_monthly/*;do
			...
			echo octave medisagg.m "${obs//$ME\/}" "${monthly//$ME\/}" "${daily//$ME\/}" >> octaves;
	...
	"4")
		...
		for f in $forecast_rslts/*;do 
			if [[ -s "$f" ]];then
				echo ${f//$ME\/} >> $rsltsf && sed 's/^/\t/g' $f >> $rsltsf;
			fi
		done
		MSG "R2 < 0.75 ==> ${rsltsf//$ME\/}";
		##############################################################
		MSG "Done";
	;;
	*)
...
# yes $dummy|head -$((L-1))|xargs > tmp.tmp;
# pr -mts" " ../1_downscaling/tmp/cptresults/CV_$g*_$L.txt|awk '$1*1>1960'|sed 's/\t/ /g'|cut -d" " -f$(seq -s, 2 2 24) >> tmp.tmp;
# tr '\n' ' ' <tmp.tmp|awk '{for(;++i<=NF;)printf $i (i%12?" ":"\n")}'|sed '$d' > f.monthly.data_$g"_"$L;

# octave medisagg.m xtmp/observed/daily/data_$g f.monthly.data_$g"_"$L f.daily.data_$g"_"$L


# yes 9999|head -$((L-1))|xargs > tmp.tmp
# tr '\n' ' ' <tmp.tmp|awk '{for(;++i<=NF;)printf $i (i%12?" ":"\n")}'|sed '$d'
# octave medisagg.m xtmp/observed/daily/data_$g xtmp/forecast/monthly/data_$g"_"$L tsts_$g"_"$L
